...
public class Log extends AbstractClass {
	...
	public Numbers evaluate(Environment env) {
		...
		res = new DoubleNumbers(Math.log(expr.evaluate(env).getDoubleValue()));
		return res;
	}
	...
	public String toStringLab() {
		return "log(" + expr.toStringLab() + ")|" + lab + "|";
	}
	...
	public String toStringRes(Map<String, Integer> env) {
		...
		;
		if (fixedMode != 0) {
			return "log(" + expr.toStringRes(env) + ")";
		} else
			return "log(" + expr.toStringRes(env) + ")|" + val + "|";
	}
	...
	public String toStringFix(Map<String, Integer> env) {
		...
		// String s = "fx_xtox("+expr.toStringFix(env)+","+f1+","+f+")";
		// return "fx_sinx(" + s + "," + f+ ")";
		String ss = "fx_subx(" + s + ",fx_ftox(1.0," + f + "))";
		String sss = "fx_subx(" + ss + ",fx_mulx(fx_mulx(" + ss + "," + ss + "," + f + "),fx_ftox(0.5," + f + ")," + f
				+ "));" + " //" + "fx_log(" + s + "," + f + ")\n";
		return sss;
	}
	...
	public void Generate_Constraints_GLPK(Map<String, String> varEnv) {
		...
		ExpressionConstraint minus = new CstrPlus(new CstrId(accd), new CstrConst(prec));
