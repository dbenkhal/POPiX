...
public class GreaterEqual extends AbstractClassB {
	AbstractClass left;
	AbstractClass right;

	public GreaterEqual(AbstractClass lt, AbstractClass rt) {
		left = lt;
		right = rt;
	...
	public Boolean evaluate(Environment env) {
		return left.evaluate(env).GREATEREQUAL(right.evaluate(env));
	}
