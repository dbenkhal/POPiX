...
public class Greater extends AbstractClassB {

	AbstractClass left;
	AbstractClass right;

	public Greater(AbstractClass lt, AbstractClass rt) {
		left = lt;
		right = rt;
	...
	public Boolean evaluate(Environment env) {
		return left.evaluate(env).GREATER(right.evaluate(env));
	}
	...
	public String toStringRes(Map<String, Integer> env) {
		return left.toStringRes(env) + symbol() + right.toStringRes(env);
	}
